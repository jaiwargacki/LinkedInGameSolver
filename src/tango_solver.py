...
class Configuration:
    def __init__(self, squares=None, rules=None, next_row=0, next_col=0):
    ...
    def make_move(self, position, value):
        ...
        if self.squares[position.row][position.col] is None:
    ...
    def validate_row_window(self, row, col):
        total_moons = 0
        total_suns = 0
        for i in range(col, min(col + 3, GRID_SIZE)):
            if self.squares[row][i] == MOON:
                total_moons += 1
            elif self.squares[row][i] == SUN:
                total_suns += 1
        if total_moons > 2 or total_suns > 2:
            return False
        else:
            return True
    ...
    def validate_col_window(self, row, col):
        total_moons = 0
        total_suns = 0
        for i in range(row, min(row + 3, GRID_SIZE)):
            if self.squares[i][col] == MOON:
                total_moons += 1
            elif self.squares[i][col] == SUN:
                total_suns += 1
        if total_moons > 2 or total_suns > 2:
            return False
        else:
    ...
    def validate_row(self, row):
        ...
        for col in range(GRID_SIZE):
            if self.squares[row][col] == MOON:
                ...
                total_suns += 1
        if total_moons > 3 or total_suns > 3:
            return False
        else:
            return True
    ...
    def get_next_row_col(self, row, col):
        ...
        if self.squares[next_row][next_col] is None:
    ...
    def get_next_configurations(self):
        ...
        next_row, next_col = self.get_next_row_col(self.next_row, self.next_col)
        if next_row >= GRID_SIZE:
            return []
        ...
        next_sun = Configuration(copy.deepcopy(self.squares), self.rules, next_row, next_col)
        next_sun.make_move(Position(next_row, next_col), SUN)
        if next_sun.is_valid():
            result.append(next_sun)
